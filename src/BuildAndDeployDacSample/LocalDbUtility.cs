...
namespace BuildAndDeployDacSample
{
  ...
  /// </summary>
  /// <remarks>
  /// Need MartinCostello.SqlLocalDb" nuget pachage and 
  /// Installing SQL Server LocalDB (https://docs.microsoft.com/en-us/sql/database-engine/configure-windows/sql-server-express-localdb?view=sql-server-ver15)
  /// </remarks>
  public static class LocalDbUtility
  {
    ...
    public static void CreateInstance(string instanceName, string version, bool isRecreate)
    {
      ...
      if (sqlLocalDbApi.GetInstanceNames().Contains(instanceName))
      {
        if (isRecreate)
        {
          // For details of options, see API Reference : https://docs.microsoft.com/en-us/sql/relational-databases/express-localdb-instance-apis/sql-server-express-localdb-reference-instance-apis?view=sql-server-ver15
